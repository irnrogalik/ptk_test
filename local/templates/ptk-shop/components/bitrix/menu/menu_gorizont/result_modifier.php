                ...
                while ($arSections = $dbSections->GetNext()) {
                    ...
                    if ($pictureSrc)
                        $arResizePicture = CFile::ResizeImageGet(
                                        $arSections["PICTURE"], array("width" => 240, 'height' => 700), BX_RESIZE_IMAGE_PROPORTIONAL, true
                        );
...
foreach ($arResult["ALL_ITEMS"] as $key => &$allItem) {
    if ($allItem["SELECTED"] != 1) {
        continue;
    }
    if ($allItem["IS_PARENT"] == 1) {
        foreach ($arResult["MENU_STRUCTURE"][$key][1] as $key => $structure) {
            if ($arResult["ALL_ITEMS"][$key]["SELECTED"] != 1) {
                continue;
            } else {
                $allItem["SELECTED"] = 0;
            }
