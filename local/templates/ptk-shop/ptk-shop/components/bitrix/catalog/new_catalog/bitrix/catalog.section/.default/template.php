...
if (!$arResult["ID"] == 3) {
    ...
    <? if (isset($arResult["SECTION_INFO"])) { ?>
        <p><?= $arSect["NAME"]; ?></p>
        <p><?= $arSect["UF_TEXT_1"]; ?></p>
        <p><?= $arSect["UF_TEXT_2"]; ?></p>
        <p><?= $arSect["UF_NOTE_SECTION"]; ?></p>
        <p><?= $arSect["UF_FACTOID_TEXT"]; ?></p>
        <p><?= $arSect["UF_FACTOID_PICTURE"] ?></p>;
    <? }
    ?>
    ...
    <div class="catalog-section bx-<?= $arParams['TEMPLATE_THEME'] ?>" data-entity="<?= $containerName ?>">
        ...
        if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS'])) {
            ...
            $APPLICATION->IncludeComponent(
                    'bitrix:catalog.item', 'item_ptk', array(), $component, array('HIDE_ICONS' => 'Y')
            );
        }
        ?>
    </div>
