...
<div class="catElement__wrap wrap n_page justified_container js_complex">
    ...
    if ($arResult["PROPERTIES"]["NEW_CARD"]["VALUE_ENUM_ID"] == 20) {
        ...
        <main class="catElement__main">
            ...
            if (isset($arResult["PROPERTIES"]["APPLICATION"]["VALUE"]) || isset($arResult["PROPERTIES"]["FIRE_PROTECTION"]["VALUE"])) {
                ?>
                <div class="n_box">
                    <h2 class="n_title n_title-middle"><?= $arResult["PROPERTIES"]["APPLICATION"]["NAME"] ? $arResult["PROPERTIES"]["APPLICATION"]["NAME"] : $arResult["PROPERTIES"]["FIRE_PROTECTION"]["NAME"] ?></h2>
                    <ul class="n_list">
                        <?
                    ...
                    <div class="n_factoid-circ">
                        <div class="circ__content circ__content-single">
                            <img src="/bitrix/templates/ptk-shop/img/n_img/pro-brite.png" alt="Pro-Brite">
                        </div>
                    </div>
                </div>
            <? } ?> 
                    ...
                    <div class="n_table">
                        <? foreach ($arResult["DISPLAY_PROPERTIES"] as $arDisplay) { ?>
                            <div class="n_table__row">
                                <div class="n_table__cell"><?= $arDisplay["NAME"] ?></div>
                    ...
                    <div class="n_list_wrap">
                        ...
                        <div class="n_factoid-circ">
                            <div class="circ__content circ__content-multi">
                                <img class="circ_ico" src="/bitrix/templates/ptk-shop/img/n_img/n_fire.svg" alt="Pro-Brite">
                                <p class="circ_text">Пожаро- и&nbsp;взрыво- безопасный</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            if (isset($arResult["PROPERTIES"]["FEATURES"]["VALUE"]) || isset($arResult["PROPERTIES"]["FEATURES_MONTAGE"]["VALUE"])) {
                ?>
                <div class = "n_box">
                    <h2 class = "n_title n_title-middle"><?= $arResult["PROPERTIES"]["FEATURES"]["NAME"] ? $arResult["PROPERTIES"]["FEATURES"]["NAME"] : $arResult["PROPERTIES"]["FEATURES_MONTAGE"]["NAME"] ?></h2>
                    <div class = "n_text">
            ...
            <? if ($arResult["PROPERTIES"]["CERTIFICATES"]["VALUE"]) { ?>
                <div class="n_box">
                    ...
                    <p class="n_text"><?= $arResult["PROPERTIES"]["CERTIFICATES"]["VALUE"] ?></p>
                    <div class="n_text">
    ...
    <? } else {
        ...
        <!--                <div class = "fix_area js_fix_area">
                        ...
                        <form action="/requests/consul.php" method="post" class="back js_validate js_back">
                            ...
                            <input type="hidden" name="from" value="http://<?= $_SERVER["HTTP_HOST"] ?><?= $_SERVER["REQUEST_URI"] ?>">
    ...
    <aside class="catElement__aside js_fix_area">
        <div class="catElement__info ">
            <div class="n_info__img"><img src="<?= $arResult["DETAIL_PICTURE"]["SRC"] ?>" alt="<?= $arResult["DETAIL_PICTURE"]["ALT"] ?>"></div>           
            <? if ($arResult["MIN_PRICE"]["VALUE"] || $arResult["OFFERS"]) { ?>
                ...
                <div class="n_info__offers">
                    <? if ($arResult["OFFERS"]) { ?>
                        <p class="n_offers__title"><?= $arResult["OFFERS"]["PROPERTIES"]["AVAILABILITY"]["NAME"] ?></p>
                        <?
                    } else {
                        foreach ($arResult["PROPERTIES"] as $prop) {
                            if (!in_array($prop["CODE"], $need_prop))
                                continue;
                            if (!empty($prop["VALUE"])) {
                                ?>
                        ...
                        <ul class="n_offers__items">
                            ...
                            foreach ($arResult["OFFERS"] as $key => $arOffer) {
                                ...
                                <li class="n_offers__item js_n-offer <?= $arOffer["PROPERTIES"]["AVAILABILITY"]["VALUE_XML_ID"] == "Y" ? "" : "disabled" ?> <?= $active ? "active" : "" ?>" 
                                    ...
                                    <span class="n_offers__item_text"><?= $arOffer["PROPERTIES"]["VOLUME"]["VALUE"] ?></span>
                                </li>                          
                            <? } ?>
                        </ul>
                    <? } ?>
                ...
                <div class="n_to_basket_wrap">
                    <form action="" class="n_to_basket_info n_hidden" id="info_to_basket">
                    ...
                    <a href="#link_to_basket_here" class="n_button n_button-go n_hidden">Оформить заявку</a>
            ...
            <div class="n_general_sideinfo">
                ...
                <p class="n_sideinfo__text n_sideinfo__tel"><a href="tel:+375291234567">+375 (29) 123-45-67</a></p>
                <p class="n_sideinfo__text n_sideinfo__tel"><a href="tel:+375291234567">+375 (29) 123-45-67</a></p>
            </div>
        </div>
    </aside>
